<template>
  <transition name="m-message-fade" mode="in-out">
    <div v-show="show" class="m-message-wrapper" :style="{ zIndex: wrapperzIndex }" :class="{ 'm-message-center': align === 'center' }">
      <div :class="['m-message', type && 'm-message-' + type]" @mouseenter="clearTimer" @mouseleave="startTimer">
        <div class="m-message-wrap">
          ...
          <div class="m-message-content">
            <span>{{message}}</span>
            <button class="m-message__close" @click="close" v-if="showClose">
              <span>×</span>
            </button>
          </div>
        </div>
